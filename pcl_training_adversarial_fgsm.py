    ...
    for epoch in range(args.max_epoch):
        ...
        if args.eval_freq > 0: # and (epoch + 1) % args.eval_freq == 0 or (epoch + 1) == args.max_epoch:
            ...
            if acc>88 and epoch>args.max_epoch-150:
                ...
                torch.save(state_, 'Models_PCL_AdvTrain_FGSM/CIFAR10_PCL_AdvTrain_FGSM_'+str(args.weight_conprox)+'_'+str(epoch)+'_'+str(float(acc))+'.pth.tar')
...
def train(model, criterion_xent, criterion_prox_1024, criterion_prox_256,
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        if args.weight_conprox != 0:
            loss = loss_xent + loss_prox_1024 + loss_prox_256 - loss_conprox_1024 - loss_conprox_256  # pcl and fgsm
        else :
            loss = loss_xent  # only fgsm
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            print(
                "Batch {}/{}\t Loss {:.6f} ({:.6f})  XentLoss {:.6f} ({:.6f})  ProxLoss_1024 {:.6f} ({:.6f}) "
