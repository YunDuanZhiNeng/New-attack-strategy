...
parser.add_argument('--lr_prox', type=float, default=0.5, help="learning rate for Proximity Loss")
parser.add_argument('--weight-prox', type=float, default=1, help="weight for Proximity Loss")
parser.add_argument('--lr_conprox', type=float, default=0.00001, help="learning rate for Con-Proximity Loss")
parser.add_argument('--weight-conprox', type=float, default=0.00001, help="weight for Con-Proximity Loss")
...
def attack(model, criterion, img, label, epoch, eps, attack_type, iters):
    adv = img.detach()
    ...
    for j in range(iterations):
        ...
        if attack_type == 'mim':
            ...
            noise = adv.grad
        if epoch < 100:
            adv.data = un_normalize(adv.data) + step * noise.sign() * epoch / 100.
        else:
            # Optimization step
...
def main():
    ...
    for epoch in range(args.max_epoch):
        ...
        if args.eval_freq > 0:  # and (epoch+1) % args.eval_freq == 0 or (epoch+1) == args.max_epoch:
            ...
            if acc > 86 and epoch > args.max_epoch - 250:
                ...
                torch.save(state_, 'Models_PCL_AdvTrain_PGD/CIFAR10_PCL_AdvTrain_PGD_iter_2' +
                           str(args.weight_conprox) + '_' + str(epoch) + '_' + str(float(acc)) + '.pth.tar')
...
def train(model, criterion_xent, criterion_prox_1024, criterion_prox_256,
    ...
    for batch_idx, (data, labels) in enumerate(trainloader):
        ...
        adv = attack(model, criterion_xent, data, labels, epoch, eps=eps, attack_type='pgd',
                     iters=2)  # Generates Batch-wise Adv Images
        ...
        if args.weight_conprox != 0:
            loss = loss_xent + loss_prox_1024 + loss_prox_256 - loss_conprox_1024 - loss_conprox_256  #pcl and pgd
        else:
            loss = loss_xent # only pgd
        ...
        if (batch_idx + 1) % args.print_freq == 0:
            print("Batch {}/{}\t Loss {:.6f} ({:.6f})  XentLoss {:.6f} ({:.6f})  ProxLoss_1024 {:.6f} ({:.6f}) "
